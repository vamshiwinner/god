...
class App extends Component {
  ...
  renderActiveItemsList = activeTabId => {
    const filterredItems = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )

    return filterredItems.map(eachItem => {
      const onClickTabItemImage = () => {
        const {activeImage} = this.state
        if (activeImage.id === eachItem.id) {
        ...
        this.setState({activeImage: randomImageUrl})
      }
      return (
        <li
          key={eachItem.id}
          onClick={onClickTabItemImage}
          className="list-items"
        >
          <button value={eachItem.category} type="button">
            <img
              className="active-images"
  ...
  gameRunningView = () => {
    const {activeTabId, activeImage, time} = this.state
    console.log(time)
    if (time === 0) {
      this.setState({isGameRunning: false})
      clearInterval(this.timerId)
      ...
      <div>
        ...
        <ul className="tab-list-items">
          {tabsList.map(eachList => {
            const onClickTab = () => {
              this.setState({activeTabId: eachList.tabId})
            }

            return (
              <li onClick={onClickTab} className="list" key={eachList.tabId}>
                <button type="button">{eachList.displayText}</button>
              </li>
            )
          })}
        </ul>
        <ul className="active-ul-container">
          {this.renderActiveItemsList(activeTabId)}
        </ul>
      </div>
    )
  }
  ...
  gameOverView = () => {
    ...
    const onClickButton = () => {
      this.setState({isGameRunning: true, time: 60, count: 0})
    }
  ...
  render() {
    const {time, count, isGameRunning} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        <div className="match-game-container">
          {isGameRunning ? this.gameRunningView() : this.gameOverView()}
        </div>
      </div>
    )
  }
